...
class PFEngine:
    ...
    def initialize_particles(self, num_particles):
        ...
        particles[:, 0] = np.random.uniform(
            start_pose_center_x - start_pose_width_by_2,
            start_pose_center_x + start_pose_width_by_2,
            num_particles,
        )
        particles[:, 1] = np.random.uniform(
    ...
    def scan_update(self, tree_msg):
        ...
        if tree_msg["positions"] is not None:
            ...
            self.particle_weights = self.get_particle_weight_localize(
                self.particles, tree_global_coords, widths_sense, postions_sense
            )
    ...
    def get_particle_weight_localize(
        self, particle_states, sensed_tree_coords, widths_sensed, positions_sensed
    ) -> np.ndarray:
        """
