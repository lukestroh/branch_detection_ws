...
def launch_setup(context: LaunchContext, *args, **kwargs):
    ...
    filepath_sensor_config = os.path.join(dir_runtime_package, "config", sensor_type.perform(context) + ".yaml")
...
def generate_launch_description():
    ...
    declared_args.append(
        DeclareLaunchArgument(
            name="serial_port", default_value="/dev/ttyACM0", description="Port name for serial device."
        )
    )
    declared_args.append(
        DeclareLaunchArgument(
    ...
    declared_args.append(
        DeclareLaunchArgument(name="num_sensors", default_value="2", description="Number of sensors being used")
    )
