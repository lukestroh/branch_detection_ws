...
class ParticleFilterNode(TFNode):
    def __init__(self) -> None:
        ...
        # self.tf_frames = []
        self.tof0_frame = (
            self.declare_parameter("tof0_frame", value="tof0").get_parameter_value().string_value
        )  # TODO: dynamically assign frames
        self.tof1_frame = self.declare_parameter("tof1_frame", value="tof1").get_parameter_value().string_value

        # Publishers
        self._pub_particles = self.create_publisher(msg_type=Marker, topic="tof_particles", qos_profile=1)

        # Timers
        self._timer_pub_particles = self.create_timer(timer_period_sec=1 / 20, callback=self._timer_cb_pub_particles)
        self._timer_get_transform = self.create_timer(timer_period_sec=1 / 20, callback=self._timer_cb_get_transform)
    ...
    def _timer_cb_get_transform(self) -> None:
        ...
        """
        try:
            self.tof0_world_transform: np.ndarray = self.lookup_transform(
                target_frame="world", source_frame=self.tof0_frame, time=self.get_clock().now(), as_matrix=True
            )
            self.tof1_world_transform: np.ndarray = self.lookup_transform(
                target_frame="world", source_frame=self.tof1_frame, time=self.get_clock().now(), as_matrix=True
            )
