...
class PyBUtils:
    def __init__(self, renders: bool = False, cam_height: int = 224, cam_width: int = 224) -> None:
    ...
    def _setup_pybullet(self) -> None:
        ...
        self.proj_mat = self.pbclient.computeProjectionMatrixFOV(
            fov=60, aspect=self.cam_width / self.cam_height, nearVal=self.near_val,
            farVal=self.far_val)

        self.pbclient.resetDebugVisualizerCamera(cameraDistance=1.06, cameraYaw=-120.3, cameraPitch=-12.48,
    ...
    def create_wall_with_texture(self, wall_dim: List, wall_pos: List, euler_rotation: List, wall_texture: int):
        wall_viz = self.pbclient.createVisualShape(shapeType=self.pbclient.GEOM_BOX, halfExtents=wall_dim,
                                                )
        wall_col = self.pbclient.createCollisionShape(shapeType=self.pbclient.GEOM_BOX, halfExtents=wall_dim,
                                                    )
        wall_id = self.pbclient.createMultiBody(baseMass=0, baseVisualShapeIndex=wall_viz,
    ...
    def add_sphere(self, radius: float, pos: List, rgba: List) -> int:
        ...
        sphereUid = self.pbclient.createMultiBody(0.0, colSphereId, visualShapeId,
                                                pos, [0, 0, 0, 1])
        return sphereUid
    ...
    def get_image_at_curr_pose(self, type, view_matrix=None) -> List:
        ...
        if type == 'robot':
            ...
            return self.pbclient.getCameraImage(width=self.cam_width, height=self.cam_height, viewMatrix=view_matrix,
                                            projectionMatrix=self.proj_mat,
                                            renderer=self.pbclient.ER_BULLET_HARDWARE_OPENGL,
                                            flags=self.pbclient.ER_NO_SEGMENTATION_MASK, lightDirection=[1, 1, 1])
        elif type == 'viz':
            return self.pbclient.getCameraImage(width=self.cam_width, height=self.cam_height,
    ...
    def setup_bird_view_visualizer(self):
        self.viz_view_matrix = self.pbclient.computeViewMatrixFromYawPitchRoll(cameraTargetPosition=[-0.3, -0.06, 1.3],
                                                                            distance=1.06,
                                                                            yaw=-120.3, pitch=-12.48, roll=0,
                                                                            upAxisIndex=2)
        self.viz_proj_matrix = self.pbclient.computeProjectionMatrixFOV(fov=60,
