...
class TwoParabolaFitter:
    ...
    def __init__(self, far_plane_threshold: float = 0.25, 
    ...
    def find_minima_with_wraparound(self, angles: np.ndarray, tof_data: np.ndarray, 
                                   original_indices: np.ndarray) -> List[Dict]:
        """
    ...
    def cluster_minima_by_angular_proximity(self, minima: List[Dict]) -> List[Dict]:
        ...
        features = np.column_stack([
            np.cos(angles),
            np.sin(angles),
            [m['tof'] for m in minima]  # Include TOF for tie-breaking
        ])
        
        # Use DBSCAN with custom metric for angular clustering
        clustering = DBSCAN(
            eps=0.3,  # Adjust based on angular_tolerance
            min_samples=1,
            metric='euclidean'
        ).fit(features)
    ...
    def unwrap_angles_for_section(self, angles: np.ndarray, 
    ...
    def segment_data_by_minima(self, angles: np.ndarray, tof_data: np.ndarray,
                              original_indices: np.ndarray, 
                              minima: List[Dict]) -> Dict[str, Dict]:
        """
    ...
    def _segment_two_minima(self, angles: np.ndarray, tof_data: np.ndarray,
                           original_indices: np.ndarray, 
                           minima: List[Dict]) -> Dict[str, Dict]:
    ...
    def _segment_multiple_minima(self, angles: np.ndarray, tof_data: np.ndarray,
                                original_indices: np.ndarray, 
                                minima: List[Dict]) -> Dict[str, Dict]:
        ...
        for i, m1 in enumerate(minima):
            for j, m2 in enumerate(minima[i+1:], i+1):
                dist = self.angular_distance(m1['angle_normalized'], 
                                           m2['angle_normalized'])
    ...
    def fit_two_parabolas(self, angles: np.ndarray, tof_data: np.ndarray,
                         original_indices: Optional[np.ndarray] = None) -> Dict:
        """
        ...
        if len(minima) < 2:
            return {
                'success': False,
                'error': f'Insufficient minima found: {len(minima)}',
                'minima': minima
            }
        ...
        # Step 3: Segment data
        try:
            segments = self.segment_data_by_minima(angles, tof_data, 
                                                 original_indices, clustered_minima)
        except Exception as e:
            return {
                'success': False,
        ...
        for seg_name, seg_data in segments.items():
            fit_result = self.fit_parabola(seg_data['angles_unwrapped'], 
                                         seg_data['tof_data'])
    ...
    def plot_results(self, angles: np.ndarray, tof_data: np.ndarray, 
                    results: Dict, title: str = "Two Parabola Fit"):
        ...
        for i, minimum in enumerate(results['minima']):
            ax1.scatter(minimum['angle'], minimum['tof'], 
        ...
        for i, (seg_name, seg_data) in enumerate(results['segments'].items()):
            ax1.scatter(seg_data['angles'], seg_data['tof_data'], 
        ...
        for i, (seg_name, seg_data) in enumerate(results['segments'].items()):
            ...
            if fit_result is not None:
                ...
                ax2.scatter(seg_data['angles_unwrapped'], seg_data['tof_data'], 
                           c=colors[i], alpha=0.7, s=30, 
                           label=f'{seg_name} data')
                
                # Plot fitted parabola
                x_fit = np.linspace(seg_data['angles_unwrapped'].min(), 
                                  seg_data['angles_unwrapped'].max(), 100)
                def parabola(x, a, b, c):
                    return a * x**2 + b * x + c
                
                y_fit = parabola(x_fit, *fit_result['coefficients'])
                ax2.plot(x_fit, y_fit, c=colors[i], linewidth=2, 
                        label=f'{seg_name} fit (R²={fit_result["r_squared"]:.3f})')
                
                # Mark vertex
                ax2.scatter(fit_result['vertex_angle'], fit_result['vertex_tof'], 
                           c=colors[i], marker='*', s=150, edgecolor='black',
