...
def fit_and_process_ransac(
    t_window, zt_window, dist_window, start_ts, window_size, max_trials, min_samples, residual_threshold
):
    fit_res = fit_ransac(
        zt_window=zt_window,
        dist_window=dist_window,
...
def score_parabola_by_height_and_width(window: dict) -> dict:
    """
    ...
    def height_func(h, a, b, c, x_min, ymin):
        return h**2 + (2 * a * x_min + b - 1) * h + (a * x_min**2 + b * x_min + c - y_min)

    res = so.fmin(func=height_func, x0=x_min, args=(a, b, c, x_min, y_min), full_output=True, disp=False)
...
def filter_parabolas(windowed_data: dict) -> dict:
    ...
    for window_idx, window in windowed_data.items():
        ...
        window = score_parabola_by_height_and_width(window=window)

        # if window['height'] != 0.5 * window['width']:
        #     continue
        # import sys
        # sys.exit()
...
def get_branch_center_time_and_distance(
    ...
    fpf_data = far_plane_filter(filter_far_plane=filter_far_plane, maf_ts=mav_filter_ts, maf_data=mav_filter_data)
    if fpf_data is None:
    ...
    if debug_plot:
        ...
        if save_plot:
            pio.write_image(fig=fig, file=os.path.join(save_plot_path, f"{sensor_name}_initial_fit.svg"), format="svg")
            pio.write_html(
                fig=fig, file=os.path.join(save_plot_path, f"{sensor_name}_initial_fit.html"), auto_open=True
            )
